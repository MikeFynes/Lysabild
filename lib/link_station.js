...
function findNearestLinkStation (deviceLocation, linkStations) {
    ...
    linkStations.map(station => {
            var distance = distanceToStation([station[0], station[1]], deviceLocation);
            var power = linkStationPower(station[2], distance);
            if(bestLinkStation == null && power > 0){
                bestLinkStation = [station[0], station[1], power];
            }
